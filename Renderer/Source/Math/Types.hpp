...
struct [[nodiscard]] Vec2
{
    ...
    Vec2() = default;
    constexpr Vec2(f32 x, f32 y) : val{x, y} { }
    explicit constexpr Vec2(f32 fill) : val{fill, fill} { }
...
struct [[nodiscard]] Vec3
{
    ...
    Vec3() = default;
    constexpr Vec3(f32 x, f32 y, f32 z) : val{x, y, z} { }
    constexpr Vec3(Vec2 xy, f32 z) : val{xy.val[0], xy.val[1], z} { }
    explicit constexpr Vec3(f32 fill) : val{fill, fill, fill} { }
...
struct [[nodiscard]] Vec4
{
    ...
    Vec4() = default;
    constexpr Vec4(f32 x, f32 y, f32 z, f32 w) : val{x, y, z, w} { }
    constexpr Vec4(Vec3 xyz, f32 w) : val{xyz.val[0], xyz.val[1], xyz.val[2], w} { }
    explicit constexpr Vec4(f32 fill) : val{fill, fill, fill, fill} { }
...
struct [[nodiscard]] Quat
{
    ...
    Quat() = default;
    constexpr Quat(f32 w, f32 x, f32 y, f32 z) : val{w, x, y, z} { }
    explicit constexpr Quat(f32 fill) : val{fill, fill, fill, fill} { }
...
struct [[nodiscard]] Mat2
{
    ...
    Mat2() = default;
    constexpr Mat2(f32 x0, f32 x1, f32 x2, f32 x3) : col{{x0, x1}, {x2, x3}} { }
    constexpr Mat2(Vec2 col0, Vec2 col1) : col{col0, col1} { }
    explicit constexpr Mat2(f32 fill) : col{Vec2{fill}, Vec2{fill}} { }
    ...
    [[nodiscard]]
    constexpr f32 operator()(int row, int column) const;
    [[nodiscard]]
    constexpr f32& operator()(int row, int column);
};
...
struct [[nodiscard]] Mat3
{
    ...
    { }
    // clang-format on
    constexpr Mat3(Vec3 col0, Vec3 col1, Vec3 col2) : col{col0, col1, col2} { }
    explicit constexpr Mat3(f32 fill) : col{Vec3{fill}, Vec3{fill}, Vec3{fill}} { }
...
struct [[nodiscard]] Mat4
{
    ...
    { }
    // clang-format on
    constexpr Mat4(Vec4 col0, Vec4 col1, Vec4 col2, Vec4 col3) : col{col0, col1, col2, col3} { }
    explicit constexpr Mat4(f32 fill) : col{Vec4{fill}, Vec4{fill}, Vec4{fill}, Vec4{fill}} { }
